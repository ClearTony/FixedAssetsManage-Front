      ...
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="status" label="维修状态">
          <template v-slot="scope">
            ...
            <el-tag type="success" v-if="scope.row.status === '维修完成'">维修完成</el-tag>
        ...
        <el-table-column label="审核" align="center" width="180" v-if="user.role === 'ADMIN'">
          <template v-slot="scope">
            <el-button v-if="scope.row.status === '待审核'" size="mini" type="success" plain @click="changeStatus(scope.row, '维修中')">通过</el-button>
            <el-button v-if="scope.row.status === '待审核'" size="mini" type="danger" plain @click="changeStatus(scope.row, '拒绝')">拒绝</el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="200">
          <template v-slot="scope">
            <el-button size="mini" type="success" plain @click="changeStatus(scope.row, '维修完成')" v-if="scope.row.status === '维修中'">完成维修</el-button>
